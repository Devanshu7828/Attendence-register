...
router.post("/attendance", auth, async (req, res) => {
  ...
  if (
    standard.students[0].attendance.find(
      (el) =>
        el.day.getDate() === new Date(req.body.date).getDate() &&
    ...
    for (let i = 0; i < standard.students.length; i++) {
      ...
      student.attendance = student.attendance.concat({
        present: Boolean(req.body.attendance[i]),
...
router.post("/viewAttendance", auth, async (req, res) => {
  ...
  standard.students.forEach((el) => {
    obj.name = el.name;
    let attendance = el.attendance.find(
      (el) =>
        el.day.getDate() === new Date(req.body.date).getDate() &&
        el.day.getMonth() === new Date(req.body.date).getMonth() &&
        el.day.getFullYear() === new Date(req.body.date).getFullYear()
